...
class MessagingAgent(Agent):
    ...
    def craft_message(
        ...
        response = completion(
            model="openrouter/anthropic/claude-sonnet-4",  # Update model name for OpenRouter
            messages=[
                {"role": "user", "content": user_prompt},
            ],
            api_key=os.getenv("OPENROUTER_API_KEY"),  # Use your OpenRouter key
            api_base="https://openrouter.ai/api/v1"  # Set OpenRouter base URL
        )
